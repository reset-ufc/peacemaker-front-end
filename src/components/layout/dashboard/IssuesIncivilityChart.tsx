// src/components/layout/dashboard/IssuesIncivilityChart.tsx
...
interface IssuesIncivilityChartProps {
  repo?: string;
}

export function IssuesIncivilityChart({ repo }: IssuesIncivilityChartProps) {
  const [period, setPeriod] = useState("24h");
  const token = localStorage.getItem("access_token");

  const { data, isLoading } = useQuery<IncivilityData[]>({
    queryKey: ["incivilities-by-type", period, repo, "issue"],
    queryFn: async () => {
      const params: any = { period, repo, type: "issue" };
        ...
        {isLoading ? <Loader /> : (
          <ResponsiveContainer>
            <AreaChart data={data}>
              ...
              <YAxis stroke="#6b7280" />
              <Tooltip />
              <Area type="monotone" dataKey="count" stroke="#34d399" fill="url(#issuesGradient)" />
            </AreaChart>
          </ResponsiveContainer>
        )}
      </div>
